...
function find_python3()
    success, python_version = run_now("/usr/bin/python3 --version 2>&1")
    if success and string.match(python_version, "Python 3") then return "/usr/bin/python3 -B" end
    success, python_version = run_now("/usr/bin/python3 --version 2>&1")
    if success and string.match(python_version, "Python 3") then return "/usr/bin/python3 -B" end
...
toolchain = GCCToolchain('/usr/share/arm-gnu-toolchain-14.2.rel1-x86_64-arm-none-eabi/bin/arm-none-eabi-', 'build', FLAGS, LDFLAGS)
